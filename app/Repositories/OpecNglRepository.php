...
class OpecNglRepository
{
    ...
    public function  manageNgl(){
        ...
        $year=$this->year;
        $Q_Day=$this->getQdays();
        $streams=$this->getStream('\App\down_terminal_stream_prod',[2,3]);
    ...
    public  function  getCondensate(){
        ...
        $gasExport=$this->fixNullArray(\App\gas_export_volume_vessel::select('volume','month')->where('year',request()->year)->first());
    ...
    function getCrudeExportQuarter($production_type_id){
        $crudeExport=$this->getMonthlyData('\App\down_terminal_stream_prod',$production_type_id);
        return $this->converToQuarterBopd($crudeExport);
    }


    function getMonthlyData($model,$production_type_id=2){
...
//stream_id
    function getMonthlyDataStream($model,$production_type_id=2){
    ...
    function getCrudeCalcStream($production_type_id){
        $down_terminal_stream_prod=$this->getMonthlyDataStream('\App\down_terminal_stream_prod',$production_type_id);
